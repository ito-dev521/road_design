...
try {
    ...
    $stmt = $pdo->prepare("DELETE FROM phases");
    $stmt->execute();
    echo "✓ 既存のフェーズデータを削除しました<br>";
    ...
    $phasesData = [
        ['設計条件の整理', '設計条件の整理と確認', 1, 1]
    ];
    
    foreach ($phasesData as $phase) {
        $phaseInsertStmt->execute($phase);
    }
    echo "✓ フェーズデータを作成しました<br>";
    ...
    $stmt = $pdo->query("SELECT id, name, description, order_num FROM phases ORDER BY order_num");
    ...
    $stmt = $pdo->query("SELECT id, phase_name, task_name, content, task_order FROM task_templates ORDER BY task_order");
    ...
    echo "<h3>完了</h3>";
    echo "<p>データベースの更新が完了しました。以下のURLで確認してください：</p>";
    echo "<ul>";
